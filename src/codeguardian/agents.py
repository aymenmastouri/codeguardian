...
def build_devops_engineer(llm: LLM, tools=None) -> Agent:
    return Agent(
        ...
        goal="Verify the stability of the build pipeline and the integrity of the integration environment. Detect build systems automatically and execute builds. Report any build failures or test failures to ensure the codebase is in a deployable state. You are the gatekeeper - you verify but do not fix code.",
        backstory="You are a battle-hardened DevOps Lead who views a broken build as a critical emergency. You specialize in build automation (Gradle, Maven, NPM) and containerization. You have a deep understanding of dependency management and can diagnose obscure linker/compiler errors in seconds. You act as the gatekeeper of the repository's health - you verify quality but send broken code back to the engineer.",
        llm=llm,
        verbose=True,
        allow_delegation=False,
        max_iter=15,
